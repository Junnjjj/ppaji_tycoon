 ...
 * ## 게이트 4 는 **지금은 경고**다
 *
 * 실측 시설 **66/75** 가 빨갛다 (지면은 33/33 통과 — `geometryGate` 주석 참조).
 * 이걸 곧바로 실패로 만들면 `npm run gate` 가 죽어서 **에셋과 무관한 작업까지 전부
 * 막힌다** — 계획서 5단계(재생성)가 그 66종을 고치는 일이고, 그때까지는 게이트가
 * **목록을 내는 일**을 한다.
 *
