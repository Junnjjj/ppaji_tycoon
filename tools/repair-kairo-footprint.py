...
⚠⚠ 한계 — **이 도구의 `--adopt` 를 목록 전체에 다시 돌리지 말 것** (2026-08-22 실측) ⚠⚠

2026-08-22 에 `--all-failing --adopt` 로 55종을 한 번에 수선해서 접지 위반을 56 → 0 으로
만들었는데, 사용자가 **화면에서** 회귀를 보고했다: "평상연립이나 탁구대같은경우 각도는
제대로인데 **꺼져있다**". 전/후 확대 대조로 재보니 원인이 이 파일 안에 있었다. 26종을
되돌렸다 (23종 백업 복원 + 3종 미러 전용). 갈래별로:

* ``mirror=True`` 의 **좌우 반전** — 정당하다. 축이 실제로 뒤집힌 그림을 되돌리는 것이고
  화소를 하나도 안 바꾼다 (9종 실측: `footprint-mirror.png` 이 뒤집은 백업과 **바이트
  동일**). 유지할 것.
* 아래 **contour 위로 잘라내는 분기** — 대체로 정당하지만 **타일 밖으로 뻗은 부속**
  (캐러밴 바퀴 · 트램펄린 앞 테두리 · 분수 앞면 석벽 · 매표소 개찰구)을 잘라 낸다.
  잘라낸 양이 원본의 **10% 를 넘으면** 그림이 물건을 잃는다 (실측: `trampoline_w` 17.5%
  · `fountain` 15.0% · `sunbed_row` 11.5% · `pool_lazy` 10.4%).
* ⚠ **마스크를 단색으로 채우는 분기 — 이것이 "가라앉아 보인다"를 만든다.** 스타일 계약은
  *"every object carries a baked 1-pixel dark warm outline"* 인데, 이 분기는
  `material_colour()` 가 고른 **한 색**으로 정본 다이아몬드를 메우고 **아래 테두리 한 줄만**
  아웃라인을 긋는다. 그래서 새로 생긴 실루엣의 **옆·윗 가장자리에는 아웃라인이 없다**
  (실측 새 가장자리 아웃라인 34~100%, 중앙 61%). 눈에는 두 가지로 보인다:
  1. **다리 사이·들린 바닥 밑의 빈 공간이 메워진다.** 탁구대·평상연립·전망대·정자·잔교가
     그렇게 죽었다 — 다리가 사라지고 물건이 어두운 판에 잠긴 것처럼 읽힌다.
  2. **물건 밖으로 아웃라인 없는 색판이 삐져나온다.** `material_colour()` 는 하단 밴드의
     최빈색을 고르는데 그게 물건의 **재질색**이면(오리보트의 흰색 · 눈썰매의 크림색)
     배경 위에 찢어진 종이 같은 조각이 생긴다.

  판정에 쓴 지표는 **`채운 화소 ÷ 정본 다이아몬드 넓이`** 다. 눈으로 나쁘다고 판정한 것은
  전부 **10% 이상**이었고(최대 `fishing` 37.2%), 괜찮은 것은 그 아래였다. 다만 10% 를
  넘어도 **이미 있던 받침대를 메운 경우**(`rent_sup` 10.4%, 빈 공간 채움 비율 29%)는
  멀쩡했다 — 그래서 **단일 문턱으로 자동 판정하지 말고 전/후 확대를 눈으로 볼 것.**
