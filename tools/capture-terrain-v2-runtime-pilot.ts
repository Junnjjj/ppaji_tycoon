/** 컨셉 충실 지면 후보를 같은 C 런타임 좌표에서 비교 캡처한다. */
...
const OUT = resolve('artifacts/asset-concept-sheets/terrain-v2-pilot/runtime-map');
...
async function prepareTerrainStage(page: Page): Promise<unknown> {
  return page.evaluate(() => {
    ...
    // B의 실제 구성을 재현한다: 굽은 모래 해안, 넓은 강, 작은 3단 언덕, L자 판석길.
    ...
    // B처럼 넓은 평지 위에 작고 명확한 3단 테라스를 만든다.
    for (let j = 12; j <= 23; j++) {
    ...
    // 두 칸 폭 판석길을 한 번 꺾어 B의 관리 시뮬 경로 실루엣을 맞춘다.
    for (let j = 10; j <= 20; j++) {
...
async function main(): Promise<void> {
  ...
  const evidence: Record<string, unknown> = {
    schemaVersion: 1,
    status: 'TERRAIN_V2_RUNTIME_REVIEW_ONLY',
    baseUrl: BASE,
  ...
  const variants = [
    ...
    { id: 'c-terrain-v2', query: '?debug=1&hd=1&hdFit=1&terrain=v2' },
  ] as const;
