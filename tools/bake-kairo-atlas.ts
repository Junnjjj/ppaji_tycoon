 ...
 * ## 왜 굽나
 *
 * `assets/generated/` 는 `.gitignore` 대상이고 **수백 MB 짜리** 작업 폴더다 (실측
 * 538M — 정확한 수를 적어 두면 배치를 돌릴 때마다 낡는다). 게임이 그걸 직접 읽을 수는
 * 없다. 그리고 144번 요청하면 폰에서 첫 프레임이 늦는다 — 한 장 + 색인 하나면
 * 요청이 **둘**이다.
 *
 * ## 왜 새 의존성이 없나
 *
...
function main(): void {
   ...
   * **높이 1147** 이 나온다. 그때는 2048 로 넓힌다 — 높이가 **494** 로 떨어져 캔버스
   * 픽셀이 오히려 **14% 적고**(2048×494 = 1,011,712 vs 1024×1147 = 1,174,528),
   * WebGL1 최소 보장 텍스처 크기가 2048 이라 폰에서도 안전하다.
   * ⚠ 이 세 수는 팩이 바뀌면 같이 움직인다 — `public/assets/kairo-atlas.png` 의 PNG
   * 헤더가 정본이다 (지금 2048×494).
   */
  let width = 1024;
