 ...
 *
 * ⚠ 면제가 20종을 넘으면 게이트가 아무것도 안 재는 것에 가깝다. 지금 5종이다.
 *
 * ⚠ **면제 사유는 그림을 보고 적을 것 — "덮임이 낮다"에서 유도하지 말 것.**
 * 2026-08-22 재검토에서 7종 중 **2종의 사유가 그림과 달랐다**:
 * · `shade_net` — "접지는 기둥 발 넷"이라 적혀 있었는데 그림에는 **바닥 널판이 통째로**
 *   깔려 있다 (실측 **덮임 98%**). 접지가 다이아몬드가 **아닌 게 아니라** 그 널판의
 *   투영이 틀렸다 (기울기 왼변 **0.351** · 오른변 −0.545, 정본 ±0.500 — 왼변이 30% 완만하다).
 *   면제가 진짜 결함을 가리고 있었다 (사용자가 화면에서 "완전히 틀어졌다"고 보고).
 * · `photozone` — "접지가 발 둘"이라 적혀 있었는데 그림은 **직육면체 받침대**다.
 *   아랫변 기울기가 **−0.089** (거의 수평) — 발이 둘이라 변이 없는 게 아니라
 *   받침대가 **정면뷰 사각형**으로 그려졌다.
 * 둘 다 면제에서 뺐다. 낮은 덮임은 "접지가 작다"의 증상이기도 하고 "투영이 틀렸다"의
 * 증상이기도 하다 — **어느 쪽인지는 그림을 봐야만 갈린다.**
 *
...
export const GEOM_HOLDOUT: readonly { id: string; axes: readonly GeomAxis[]; why: string }[] = [
  {
    id: 'parasol',
    axes: ['slope', 'iou'],
    why: '파라솔 **한 대 + 선베드 둘**이다 (그림 실측 — "기둥 하나"가 아니다). 접지는 파라솔 기둥 하나 + 의자 다리 넷이라 아래 두 변이 없다. **되돌리는 조건**: 이 그림에 돗자리·데크 같은 바닥판을 깔면 접지가 다이아몬드가 되므로 이 줄을 지운다.',
  },
  {
    id: 'lifering',
    axes: ['slope', 'iou'],
    why: '구명함 스탠드 + 바닥에 놓인 튜브. 다리 둘이라 접지가 점 둘이다 (실측 덮임 63%, 오른변은 아예 측정 불가). **되돌리는 조건**: 스탠드에 받침판을 붙이면 지운다.',
  },
  {
    id: 'vending_in',
    axes: ['slope', 'iou'],
    why: '자판기 — 타일보다 **작은 상자**다. 1×1 발자국을 꽉 채우면 오히려 손님 통로가 없어 보인다 (실측 덮임 63%). 접지가 작은 것이 의도다. 투영 자체는 맞다 — 왼변 기울기가 정확히 **0.500** 이다 (`shade_net`·`photozone` 과 갈리는 지점). **되돌리는 조건**: 발자국을 타일에 꽉 채우는 형태로 다시 그리면 지운다.',
  },
  {
    id: 'vending_out',
    axes: ['slope', 'iou'],
    why: '`vending_in` 과 같은 물건의 야외판(+ 쓰레기통) — 타일보다 작은 상자라 접지가 다이아몬드가 아니다. 같은 이유로 같이 면제한다 (한쪽만 면제하면 같은 그림에서 판정이 갈린다). 왼변 기울기 **0.500** 으로 투영은 맞다. **되돌리는 조건**: `vending_in` 과 같다.',
  },
  {
    id: 'arcade',
    axes: ['slope', 'iou'],
    why: '오락기 한 대 — 타일보다 작은 상자. `vending_*` 와 같은 이유. 오른변 기울기 **−0.500** 으로 투영은 맞다. **되돌리는 조건**: `vending_in` 과 같다.',
  },
];
