...
const PAIRS = [
  ...
  ['흰 글씨 · 주의', '--text-on-solid', '--risk-watch', 4.5],
  ['좋음 · 패널', '--good', '--panel', 3],
  ['나쁨 · 패널', '--bad', '--panel', 3],
  ...
  /*
   * K47-②: 위험도 칩이 하단 바에서 **헤더 2줄째**로 올라갔다 — 진한 칠 + 흰 글씨
   * 그대로다. 글씨 쪽은 위의 `흰 글씨 · 위험/경계` 두 쌍이 이미 재므로, 여기서는
   * **칩 면이 크림 패널에서 떠 보이는가**(비문자 3:1)를 나머지 두 색으로 채운다.
   * 실측(패널 대비): safe 4.14 · watch 3.93 · caution 3.65 · danger 4.92 —
   * 문턱 3 에 가장 붙어 있는 것이 caution 이다. 위험도 색을 손보면 여기부터 다시 잴 것.
   */
  ['위험도 안전 · 패널', '--risk-safe', '--panel', 3],
  ['위험도 경계 · 패널', '--risk-caution', '--panel', 3],
  /*
   * K47-①: 티커가 붓 라벨을 물고 있을 때(`.kticker.brush`)의 색 — `--accent` 글씨가
   * 하단 바와 같은 `--bar-bg` 위에 얹힌다. 이 조합은 지금까지 아무도 안 쟀다.
   */
  ['붓 라벨 · 티커 띠', '--accent', '--bar-bg', 3],
  /*
   * K51: 배치 중 입구·출구 표식. 여기는 **캔버스**라 CSS 가 안 그리지만, 색을
   * `cssVar()` 로 읽으므로(K34) 토큰이 바뀌면 지도 위 글씨가 조용히 안 읽히게 된다.
   *
   * ⚠ 세 번째 쌍이 이 셋 중 가장 위태롭다 — **면끼리**(입구 파랑 ↔ 출구 주황)의 3:1 은
   * "둘 중 어느 쪽이 입구인가"를 색만으로도 가를 수 있나를 재는 것이고, 흑백·색약에서
   * 유일하게 남는 단서다. 첫 시안(#2f6fd0/#e8951f)은 **2.03** 이라 회색으로 같았다.
   * 글씨(`입구`/`출구`)가 첫째 채널이지만 그것 하나에 기대면 폰에서 11px 두 자다.
   */
  ['입출구 글씨 · 입구', '--ride-ink', '--ride-entry', 4.5],
  ['입출구 글씨 · 출구 테두리', '--ride-ink', '--ride-exit-edge', 4.5],
  ['입구 면 · 출구 면', '--ride-entry', '--ride-exit', 3],
  /*
   * K54: 바닥 화살표는 **밝은 칠 + 진한 테두리**다 (도로 노면 표시). 글씨가 아니라
   * 도형이므로 문턱은 보조(3:1) — 다만 이 쌍이 무너지면 화살표가 통째로 한 덩어리가
   * 되어 방향을 못 읽는다. 처음 시안은 칠도 `--ride-entry` 였는데 그러면 폴리라인과
