...
최종 갱신 2026-08-27. 이 문서는 후보 에셋을 메인 브랜치와 라이브 팩에 채택할 때 다시
...
결정 상태는 `USER_APPROVED_AND_LIVE_ADOPTED`다. 2026-08-27 현재 아이스크림 1×1,
카페 2×2와 두 시설의 d0–d3 채색 스프라이트가 라이브 계약·아틀라스에 원자적으로
적용되었다.
...
3. 같은 변경에서 `src/assets/kairo-render-contract.json`의 `canvas`와 `anchorTexel`을
   위 표의 값으로 갱신한다. `bodyH: 20`은 유지한다.
