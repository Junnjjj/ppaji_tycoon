...
| `cafe` | **3×2** | **3×2** | 106×78, `[53,78]`, 좌우 guard 13 | `[0..1,0..1]` 4개 유지 | v5 라이브 적용 완료 |
| `nursing` | **2×2** | **2×2** | 72×60, `[36,60]`, 좌우 guard 4 | `[0,0]` 1개 유지 | 라이브 적용 완료 |

결정 상태는 `USER_APPROVED_AND_LIVE_ADOPTED`다. 아이스크림 1×1은 2026-08-27,
수유실 2×2와 카페 v5 3×2는 2026-09-01에 d0–d3 채색 스프라이트와 라이브
계약·아틀라스에 원자적으로 적용되었다. 카페의 2026-08-27 2×2 결정은 v5 채택으로
대체되었으며 과거 HD 파일럿 비교에서만 이력으로 남는다.

### 2026-09-01 카페 v5 3×2 라이브 채택과 비정사각 접점 보정

사용자의 카페 적용 요청으로 별도 보류했던 최신 v5를 3×2로 승인·채택했다. 논리 캔버스는
`64×52 → 106×78`, anchor는 `[32,52] → [53,78]`, bodyH는 `20 → 38`이며 좌우 13
논리 텍셀은 물리 크기를 줄이지 않는 투명 guard다. 네 프레임은 승인된 D=4 원본에서 직접
D=2 212×156으로 만들며, 이전에 거절된 `kairo-cafe-3x2-strict-color-v2`는 사용하지 않는다.

채택 직전 실제 게임 접지 게이트가 v5 런타임 패커의 비정사각 정렬 결함을 잡았다. 패커는
3×2인데도 최하단 접점을 캔버스 중앙에 놓았고, 그 결과 실제 맵에서 카페가 타일 한쪽을
침범했다. 채색·알파·형상은 바꾸지 않고 d0/d2를 `+8`, d1/d3를 `−8` 논리 텍셀만큼
캔버스 안에서 이동했다. 보정 뒤 접지 실측은 d0/d2 꼭짓점 `0.600/0.600`, d1 `0.394/0.400`,
d3 `0.400/0.400`, IoU `0.942–0.950`으로 네 방향 모두 통과했다. 정본 해시는
[`cafe-v5-3x2-live.json`](cafe-v5-3x2-live.json)에 기록한다.
...
| C | `?hd=1&hdFit=1` | 2× | 당시 승인 1×1, 2×2 + d0–d3 | **과거 비교본** |
...
C에서는 당시 실제 런타임이 `facility/icecream:d0..d3`, `facility/cafe:d0..d3` 여덟 텍스처를
...
당시 판단은 **C를 HD 픽셀 방향의 기준 후보로 유지**하는 것이었다. 이 2×2 카페 파일럿은
이후 v5 3×2 라이브 채택으로 대체되었으며 기본 URL의 현재 카페 정본이 아니다. B는 해상도 변경과
...
- 카페: 이 웨이브에서는 보류했지만 이후 최신 v5 3×2를 별도 승인해 라이브 채택 완료.
