...
## 2026-08-26 후속 경영 UX 재구성

이 문서의 UI 셸 v2 검증 뒤 `818f62e58e9dd40819e3a64155ca11bcb4e362b3`에서 홈·전체 메뉴·
성장 목록·사건 상세·건설 one-shot·코스 적용 완료·한국어/금액 표면을 후속 재구성했다.
무엇을 왜 바꿨는지, 계획과 실제 구현이 어디서 달라졌는지, 어떤 회귀 계약과 미해결 게이트가
남았는지는 `docs/ui-management-redesign-decisions.md`가 정본이다.

후속 구현에서 확인한 집중 결과는 UI Vitest 251/251, UI 표면 gate 28/28, 홈·전체 메뉴
41/41, 코스 26/26, 건설 19/19, 사고 단독 10/10이다. 전체 `npm run test`는 생성 PNG fixture
두 장 부재로 계속 통과 처리하지 않는다. 사람 A/B·설명 없는 30초 과업 승인도 여전히 별도다.
