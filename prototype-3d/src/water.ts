...
/**
 * 해안선 기울기 (dz/dx). **구도의 핵심 상수다.**
 *
 * 실측: yaw 45 아이소에서 지면 방향 dz/dx 가 화면에 찍히는 각도는
 *   1.0 → 0° (정확히 수평) · 1.6 → −7.6° · 1.8 → −9.4° · 2.0 → −10.9°
 * 레퍼런스의 잔교·해안선은 **8~12°** 다 (격자축은 최소 24° 라 축으로는 절대 안 나온다).
 * 그래서 해안선은 격자축이 아니라 **타일 공간의 대각선**이어야 한다 → 1.8.
 *
 * 0 으로 두면 해안선이 +X 축과 나란해져 화면에서 30° 로 가파르게 선다 —
 * "맵이 사선이라 이상하다"의 직접 원인이었다.
 */
// ⚠ 경사 실험(1.5~1.8)은 폐기했다. 기울어진 해안은 격자·지형·검사 전부를 해안 거리
// 좌표계로 끌고 들어가며 복잡도를 폭발시켰고, 그래도 레퍼런스처럼 안 보였다.
// 레퍼런스의 구조는 기울어진 세계가 아니라 **무대**다: 수평 물가선 + 그 앞 고정 무대.
export const SHORE_SLOPE = 0;
/** 물가선 z — 잔교(stage.ts) 뒤 절반이 모래, 앞 절반이 물에 걸치는 위치 */
export const SHORE_BASE = -60;

export function shoreLine(x: number): number {
  return SHORE_BASE + x * SHORE_SLOPE
    + Math.sin(x * SHORE_FREQ) * SHORE_AMP
    + Math.sin(x * SHORE_FREQ * 2.7 + 1.3) * (SHORE_AMP * 0.4);
}

/**
 * 해안선으로부터의 거리 — 음수면 뭍, 양수면 물.
 * 지형·숲·모래·수심 밴드를 **전부 이 값 하나로** 결정한다. 예전엔 z 를 직접 썼는데,
 * 해안선을 기울이자 숲이 한쪽에만 남는 문제가 생겼다 (땅은 기울었는데 숲은 안 기울어서).
 */
export function shoreDist(x: number, z: number): number {
  return z - shoreLine(x);
}

/**
 * 해안선을 따라가는 띠 지오메트리 — 물·모래·잔디·언덕이 **전부 이걸로** 만들어진다.
 *
 * ⚠ 축 정렬 평면을 쓰면 안 된다. 해안선이 기울어져 있어서 축 정렬 사각형은 한쪽 끝이
 * 반대편 지형으로 넘어간다 (물 평면이 뭍을 덮은 게 그 사고였다).
 *
 * @param d0,d1  해안 거리 범위 (음수 = 뭍)
 * @param halfX  x 반경 — **가로 화면까지 덮을 만큼 넉넉히**. 좁으면 띠 끝 너머로
 *               반대편 지형이 비쳐 "리본" 처럼 보인다 (실측).
 */
export function shoreStrip(d0: number, d1: number, halfX = 1800, seg = 160): THREE.BufferGeometry {
  const pts: number[] = [];
  const uvs: number[] = [];
  const idx: number[] = [];
  for (let i = 0; i <= seg; i++) {
    const x = -halfX + (2 * halfX * i) / seg;
    const s = shoreLine(x);
    pts.push(x, 0, s + d0); uvs.push(i / seg, 0);
    pts.push(x, 0, s + d1); uvs.push(i / seg, 1);
  }
  for (let i = 0; i < seg; i++) {
...
float shore(float x) {
  return ${SHORE_BASE.toFixed(1)} + sin(x * ${SHORE_FREQ}) * ${SHORE_AMP.toFixed(1)} + sin(x * ${(SHORE_FREQ * 2.7).toFixed(4)} + 1.3) * ${(SHORE_AMP * 0.4).toFixed(1)};
}
...
void main() {
  ...
  vec3 emerald   = vec3(0.361, 0.706, 0.643); // #5cb4a4 강물 얕은
  vec3 turquoise = vec3(0.247, 0.620, 0.580); // #3f9e94
  vec3 midblue   = vec3(0.180, 0.502, 0.502); // #2e8080
  vec3 cobalt    = vec3(0.145, 0.416, 0.455); // #256a74
  vec3 deep      = vec3(0.114, 0.333, 0.376); // #1d5560 강물 깊은
  ...
  vec3 lightC = band < 0.5 ? vec3(0.722, 0.894, 0.831)  // #b8e4d4
              : band < 1.5 ? vec3(0.561, 0.816, 0.737)  // #8fd0bc
              : vec3(0.247, 0.620, 0.580);              // #3f9e94
  vec3 darkC  = band < 0.5 ? vec3(0.247, 0.620, 0.580)  // #3f9e94
              : band < 1.5 ? vec3(0.145, 0.416, 0.455)  // #256a74
              : band < 2.5 ? vec3(0.114, 0.333, 0.376)  // #1d5560
              : band < 3.5 ? vec3(0.086, 0.259, 0.306)  // #16424e
              : vec3(0.071, 0.184, 0.227);              // #122f3a
...
export function makeWater(): { mesh: THREE.Mesh; material: THREE.ShaderMaterial } {
  ...
  // ⚠ 물은 두 띠다: 앞강(플레이 수면) + **뒷강**(땅 너머, 건너편 마을 앞).
  //   뒷강이 없으면 땅끝이 허공에서 끝나 배경과 못 이어진다.
  const mesh = new THREE.Mesh(shoreStrip(0, 1400, 2100, 160), material);
  const back = new THREE.Mesh(shoreStrip(-164, -136, 2100, 40), material);
  mesh.add(back);
