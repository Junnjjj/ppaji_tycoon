...
describe('골든 시나리오 — 고정 시드·고정 건설 순서', () => {
  ...
  it('12주 뒤 숫자가 못박힌 값과 같다', () => {
     ...
     *
     * **2026-08-27 승인 풋프린트 채택**: 카페가 2×3→2×2가 되어 같은 건설 좌표에서
     * 보행 동선 한 줄이 열렸다. 방문 70→76 · 거절 7→1 · 만족 62→64로 이동했고,
     * 입장료·판매액은 그 방문객 증가만큼 움직였다. 시설/콤보/카드/등급은 동일하다.
     *
     * **2026-09-01 카페 v5 3×2 라이브 채택**: 카페가 2×2→3×2가 되면서 같은 고정
     * 좌표에서 발자국·앞면 접근 칸·보행장이 함께 바뀌었다. 방문 76→93, 거절 1→28,
     * 만족 64→62, 등급 2→3, 입장료 288,800→353,400, 판매 262,399→309,643으로
     * 이동했다. 시설 15·콤보 7·카드 4·직원 급여·코스 매출·평균 레벨은 동일하고,
