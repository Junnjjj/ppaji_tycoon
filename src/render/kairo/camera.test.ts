...
describe('월드 경계 — 배경 여백을 포함한다', () => {
  ...
  it('맵 가로 폭이 1152 텍셀 — 폰 393 의 2.9 화면', () => {
    const b = worldBounds();
    expect(b.maxX - b.minX).toBe(1152);
    expect((b.maxX - b.minX) / 393).toBeCloseTo(2.93, 1);
  });

  it('맵 세로가 폰 852 에 들어간다 — 배경 여백까지 852', () => {
    const b = worldBounds();
    expect(b.maxY - b.minY).toBe(576 + BACKDROP_ABOVE + BACKDROP_BELOW);
    expect(b.maxY - b.minY).toBe(852);
  });
});
